...
writeIndex (Declarations trie) = do
  let
    ...
    insert { prefix, results } mbOldValue =
      case mbOldValue of
        Nothing -> Just $ results
        Just oldValue -> Just $ oldValue <> results

    mp = Array.foldr (\part -> Map.alter (insert part) (getPartId part.prefix)) mempty parts

  for_ (Map.toUnfoldableUnordered mp :: Array _)
