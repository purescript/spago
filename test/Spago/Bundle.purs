...
spec = Spec.around withTempDir do
  ...
  checkBundle file fixture =
    checkFixture' file fixture normalizeForFixture \actual expected ->
      Str.trim actual `shouldEqualStr` patchWatermark (Str.trim expected)
    where
    watermark version = "/* Generated by Spago v" <> version

    -- When writing fixtures: replace actual version with placeholder "0"
    normalizeForFixture =
      Str.replace
        (Str.Pattern $ watermark BuildInfo.packages."spago-bin")
        (Str.Replacement $ watermark "0")

    -- When comparing: replace placeholder "0" with actual version in expected
    patchWatermark =
      Str.replace
